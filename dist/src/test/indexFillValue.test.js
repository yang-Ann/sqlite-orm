...
test("select", () => {
    ...
    const sql4 = sqliteOrm
        ...
        .getSqlRaw();
    expect(sql4).toMatchInlineSnapshot('"SELECT * FROM \\"my_table\\" WHERE ( name=1 AND name=2 AND name=\\"hello\\" ) OR age=18 AND gex IN (1,2,3) AND ( gex!=1 AND gex!=\\"2\\" AND gex!=false )"');
...
test("insert", () => {
    ...
    expect(sql6).toMatchInlineSnapshot('"INSERT or REPLACE INTO \\"my_table\\" (name, age, gex, isFlag) VALUES (\\"张三\\", 18, \\"男\\", true)"');
});
test("inserts", () => {
